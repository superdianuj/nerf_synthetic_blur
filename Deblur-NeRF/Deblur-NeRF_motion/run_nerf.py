...
def sample_frame(video_path, output_dir, frame_interval=3):
    ...
    while True:
        ...
        if frame_count % frame_interval == 0:
            ...
            print(f'Frame {frame_count} is saved as {frame_name}')
        frame_count += 1
    cap.release()

    print(f'All frames are saved in {output_dir}')
...
def train():
    ...
    if args.render_test:
        render_poses_copy = np.array(poses)
    ...
    render_poses_copy = torch.tensor(render_poses_copy[:, :3, :4]).cuda()
    nerf = nerf.cuda()
    ...
    if args.render_only:
        ...
        with torch.no_grad():
            testsavedir = os.path.join(basedir, expname,
                                       f"renderonly"
                                       f"_{'test' if 1==0 else 'path'}"
                                       f"_{start:06d}")
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', render_poses_copy.shape)

            dummy_num = ((len(poses) - 1) // args.num_gpu + 1) * args.num_gpu - len(poses)
            dummy_poses = torch.eye(3, 4).unsqueeze(0).expand(dummy_num, 3, 4).type_as(render_poses_copy)
            print(f"Append {dummy_num} # of poses to fill all the GPUs")
            nerf.eval()
            rgbshdr, disps = nerf(
                hwf[0], hwf[1], K, args.chunk,
                poses=torch.cat([render_poses_copy, dummy_poses], dim=0),
            ...
            if 1==0 and args.render_multipoints:
                for pti in range(args.kernel_ptnum):
                    nerf.eval()
                    poses_num = len(poses) + dummy_num
                    imgidx = torch.arange(poses_num, dtype=torch.long).to(render_poses_copy.device).reshape(poses_num, 1)
                    rgbs, weights = nerf(
                        hwf[0], hwf[1], K, args.chunk,
                        poses=torch.cat([render_poses_copy, dummy_poses], dim=0),
                        render_kwargs=render_kwargs_test,
    ...
    for i in range(start, N_iters):
        ...
        if i % video_save_freq == 0 and i > 0:
            ...
            def save_video(filename,data,fps=24):
                ...
                if len(data.shape)==4:
                    ...
                    print("----FUCK-----")
                    out=cv2.VideoWriter(filename,cv2.VideoWriter_fourcc(*'XVID'),fps,size,isColor=False)
                for frame in data:
            ...
            save_video(moviebase+'rgb.mp4',rgbs,fps=24)
            save_video(moviebase+'disp.mp4',disps/disps.max(),fps=24)


            # sample_frame(moviebase+'rgb.mp4', 'deblur_nerf_output', 3)

            # os.system(f'python compute_metrics.py --gt_dir scene/images --target_dir deblur_nerf_output')


            

            #with imageio.get_writer(moviebase+'rgb.mp4') as writer:
            #    for frame in rgbs:

             #       writer.append_data((frame*255).astype(np.uint8))


            #imageio.mimwrite(moviebase + 'disp.mp4', to8b(disps / disps.max()))
        ...
        if i % video_save_freq== 0 and i > 0:    # save the rendered test poses
            ...
            with torch.no_grad():
                ...
                os.system(f'python compute_metrics.py --gt_dir specified_images --target_dir {testsavedir}')
